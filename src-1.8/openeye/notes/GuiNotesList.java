...
public class GuiNotesList extends GuiScrollingList {
	...
	protected void drawSlot(int slotId, int right, int top, int height, Tessellator tessellator) {
		...
		owner.drawString(mc.fontRendererObj, entry.title().getFormatted(), left + 20, top + 4, 0xFFFFFF);
		String description = entry.content().getFormatted();

		int width = right - left;
		if (!Strings.isNullOrEmpty(description)) mc.fontRendererObj.drawSplitString(description, left, top + 20, left + width - 10, 0xCCCCCC);
	}
}
